...
public:
	...
	int GetTrend()
	{
		...
		if(Ind.ma_k2 > Ind.ma_k3)// && Ask - low < 50 * BasePoint)
		{
	        if(Ind.ma_k1 > 0 && Ind.ma_k1 < Ind.ma_k2)
	            rtn = -1;
			else
			    rtn = 0;
	    }
		else if(Ind.ma_k2 < Ind.ma_k3)// && high - Bid < 50 * BasePoint)
		{
		    if(Ind.ma_k1 > 0 && Ind.ma_k1 > Ind.ma_k2)
	            rtn = -1;
			else
	...
	int GetCommand(int &magic, int &magicnumber, double &lots, int maxcount, int frame)
	{
			...
			case 11:
				{
					...
					if(adx < Division11 && wpr < WprOpen11 + (-100) && close < ma700 + 60 * BasePoint && trend == 0)
						cmd = 0;
					else if (adx < Division11 && wpr > -WprOpen11 && close > ma700 - 60 * BasePoint && trend == 1)
						cmd = 1;
				}
				break;
			case 12:
				{
					...
					if(rsi < 30 && close < ma700 + 60 * BasePoint && trend == 0)
						cmd = 0;
					else if ( rsi > 70 && close > ma700 - 60 * BasePoint && trend == 1)
						cmd = 1;
				}
				break;
			case 4:
				{
					if (Ind.close_m5 >= Ind.ma_h1 + Ind.atr_h1 * 1.4 + 13 * BasePoint && Ind.close_m5 < Ind.ma700_m5 + MAMargin4 * BasePoint && Ind.rsi_m5 < 70 && trend == 0)		
						cmd = 0;
					else if (Ind.close_m5 <= Ind.ma_h1 - Ind.atr_h1 * 1.4 - 13 * BasePoint && Ind.close_m5 > Ind.ma700_m5 - MAMargin4 * BasePoint	&& Ind.rsi_m5  > 30 && trend == 1)
						cmd = 1;
				}
				break;
			case 999:
		...
		if(OrderIsFull(magicnumber, cmd, maxcount)) cmd = -1;
	...
	{		
    	...
    	{		
		    ...
		    if(true && profit > 0 && Orders.size() > 0)
		    {
			    ...
			    if(LostArr.size() > 0)
			    {
				    ...
				    {  
			            ...
			            if(lots > 0)
			            {
			            	...
			            	order.Ticket = ticket;
			            	order.Lots = lots;
			            	ArrToClose.push_back(order);
			            }
			            
			            if(profit == 0) break;                               
			        }
	
					vector<OrderInfo>::iterator iter1;
				    for (iter1 = ArrToClose.begin(); iter1 != ArrToClose.end(); iter1++)  
				    {  
				    	OrderInfo order = (OrderInfo)*iter1;
