...
public:
			...
			case 999:
				{
				    ...
				    if(maxloss > BCStep)
				    {
				    	...
				    	magicnumber = 990 + maxloss / BCStep;				    	
				    	maxcount = bctimes + 2;			    	
				    	
			    		cmd = Ind.Trend ;
			    		if(cmd == 0 &&  Ind.wpr_m60 > -90) cmd = -1;
			    		if(cmd == 1 && Ind.wpr_m60 < -10) cmd = -1;
					}				    
				}	
				break;
		}
	...
	void Add2StreamOfDC(vector<OrderInfo> &LostArr, wostringstream &ss, double dcloss, bool include001 = false)
	{
		double totalloss = 0;
        vector<OrderInfo>::iterator iter; 
	    ...
	    {  
	    	OrderInfo order = (OrderInfo)*iter;
	  		if(!include001 && order.Lots == 0.01) continue;         
	...
	{		
	    ...
	    {  
			...
			if(cmd == 0 && price - Ind.bid > BCStep * BasePoint || cmd == 1 && Ind.ask - price  > BCStep * BasePoint)
	        {  		   				
				OrderInfo order;  
	            order.CMD = cmd;
	            order.Ticket = ticket;
	            ...
	            if(cmd == 0)
	            {
	            	...
	            	has0 = true;
	            	totalloss0 += order.LossPoint * order.Lots;
				}	                
	            else
	            {
	            	...
	            	has1 = true;
	            	totalloss1 += order.LossPoint * order.Lots;
				}	            
	        }   
		...
		if(has0 && has1)//天地悬空 
		{
			if(Ind.Trend == 0) 
			{
				...
				Add2StreamOfDC(LostArr1, ss, totalloss1 * PercentOf2 * times, true);
			}
			if(Ind.Trend == 1) 
			{
				int times = (max0 - Ind.ask) / BasePoint / BCStep;				
				Add2StreamOfDC(LostArr0, ss, totalloss0 * PercentOf2 * times, true);
			} 
		}
		else if(has0)//只有buy单亏 
		{
			int times = (max0 - Ind.ask) / BasePoint / BCStep;				
			Add2StreamOfDC(LostArr0, ss, totalloss0 * PercentOf1);
		} 
		else if(has1)//只有sell单亏 
		{
			int times = (Ind.bid - min1) / BasePoint / BCStep;				
			Add2StreamOfDC(LostArr1, ss, totalloss1 * PercentOf1);
		}		
	...
	void AddConfig(int rsiLimit,int division11,int wprOpen11,int maMargin4,int minMovePoint,int maxMovePoint,
		int digits, double basePoint, int maxSL, double baseLots, double bcRate, int bcStep, double percentOf1, double percentOf2)
	{
		RSILimit = rsiLimit;
		...
		BCRate = bcRate;
		BCStep = bcStep;
		PercentOf1 = percentOf1;
		PercentOf2 = percentOf2;
	}
		...
		int digits,double basePoint, int maxSL, double baseLots, double bcRate, int bcStep, double percentOf1, double percentOf2)
{
	try
	{
		trade.AddConfig(rsiLimit, division11, wprOpen11, maMargin4, minMovePoint, maxMovePoint, digits, basePoint, maxSL, baseLots, bcRate, bcStep, percentOf1, percentOf2);	
	}
	catch(...)
	{		
	}
